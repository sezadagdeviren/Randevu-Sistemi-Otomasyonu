// public/js/pending-video-call.js

document.addEventListener('DOMContentLoaded', () => {
    const videoAppointmentsTableBody = document.querySelector('#videoAppointmentsTable tbody');
    ...
    async function loadVideoAppointments() {
        try {
            const response = await fetch('/appointments?type=Görüntülü%20Görüşme&status=confirmed', {
                method: 'GET',
                credentials: 'include'
            });
            ...
            // Tabloyu Temizle
            videoAppointmentsTableBody.innerHTML = '';
            ...
            appointments.forEach(appointment => {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <button class="btn btn-info btn-sm join-meet-btn" data-link="${appointment.google_meet_link}">
                ...
                `;
                videoAppointmentsTableBody.appendChild(tr);
            });
        } catch (error) {
            console.error(error);
    ...
    videoAppointmentsTableBody.addEventListener('click', async (e) => {
    ...
    // Google Meet Linkine Gitme
    videoAppointmentsTableBody.addEventListener('click', (e) => {
        const target = e.target.closest('.join-meet-btn');
        if (!target) return;
